...
</br>Основу игры обеспечивает QuestService с функциями changeState(newState: Int) и changeQuest(newQuest: String, newState: Int = 0 они обеспечивают переход от предыдущего состояния к следующему.
</br>В игре реализованы мини игры которые позволяют получить различные бонусы при успешном прохождении.
<img width="456" alt="Пример3" src="" />
